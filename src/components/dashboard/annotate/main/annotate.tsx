...
export const AnnotationPageContext =
  React.createContext<IAnnotationPageContext>({
    curImage: null,
    labels: [],
    imageIndex: 0,
  ...
  useEffect(() => {
    if (params.datasetId) {
      (async function getCurrentDataset() {
        try {
          const datasetState = (
            await api.get(`/api/dataset/${params.datasetId}/state`)
          ).data.data;
          let images = [];
          if (datasetState === 'feedback') {
            images = (
              await api.get(`/api/dataset/${params.datasetId}/imageIds`, {
          ...
          const datasetLabels = (
            await api.get(`/api/dataset/${params.datasetId}/labels`)
          ).data.data;
          setImageIndex(0);
          setImageIds(images);
    ...
    if (imageIds.length) {
      (async function getCurrentImageInfo() {
        try {
          ...
          const fetchedImage = (
            await api.get<{ success: boolean; data: UploadedImage }>(
              `/api/image/${imageIds[imageIndex]}`
            )
          ).data.data;
          ...
          const remoteAnnotationData = (
            await api.get<{ success: boolean; data: RemoteAnnotationObject[] }>(
              `/api/image/${imageIds[imageIndex]}/annotations`
            )
          ).data.data;
          ...
          for (const remoteObject of remoteAnnotationData) {
            ...
            localAnnotationData.set(
              v4(),
              new AnnotationObject(remoteObject.class, remoteObject.id, {
                x,
                y,
                width,
                height
    ...
    <div className={ classes.annotateContainer }>
      ...
      <div className={ classes.labelContainer }>
        ...
        <div className={ classes.utilityButtons }>
          ...
          >
            {isEditing ? 'Currently Editing Boxes' : 'Add Bounding Box'}
          </Button>
        </div>
      </div>
