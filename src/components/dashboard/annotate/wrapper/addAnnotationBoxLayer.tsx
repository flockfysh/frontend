...
export default function AddAnnotationBoxLayer(props: {
    ...
    const start = React.useRef<{ sX: number, sY: number }>({sX: 0, sY: 0});
    const end = React.useRef<{ eX: number, eY: number }>({eX: 0, eY: 0});
    const internalRectRef = React.useRef<Konva.Rect>(null);
    ...
    function getRectCoords() {
        let {sX, sY} = start.current;
        let {eX, eY} = end.current;
    ...
    return (
        <Layer>
            <Rect ref={internalRectRef} stroke={'red'} strokeWidth={4} opacity={isDragging ? 1 : 0}></Rect>
            <Rect x={0} y={0} strokeScaleEnabled={false} width={props.width}
                  height={props.height} draggable
                  onDragStart={e => {
