...
async function editRecipe(id: string, params: {
    ...
    for (const label of params.labels) {
        if (label.isNew) {
            ...
            if (!label._id) {
                throw new Error('Missing label ID - cannot edit!');
            }
...
const ANNOTATION_TOOL_FULL_OPTIONS = ANNOTATION_TOOL_OPTIONS.map(opt => {
    return {
        value: opt.value,
        label: <div className={ classes.labelTool }>
            <ReactSVG className={ classes.labelToolIcon } src={ iconMapping[opt.value].src }></ReactSVG>
            <span>{opt.label}</span>
        </div>,
    };
});
...
export default function RecipeForm(props: RecipeFormProps) {
    const { close: closeForm } = React.useContext(PopupModalContext);
                    ...
                    {labels.size > 0 ? (
                        <ul className={ classes.labelDataFieldWrapper }>
                            {Array.from(labels.entries()).map(function transformEntry([clientSideUuid, label]) {
                                return (
                                    <Label { ...label } immutable={ immutable } key={ clientSideUuid } onDelete={ () => {
