...
const statusesValues: Record<Flockfysh.PullRequestStatus, {
    icon: JSX.Element,
    label: string,
    styles?: string,
}> = {
...
const StatusBadge = ({ status }: { status: Flockfysh.PullRequestStatus }) => {
...
export const ContributionItem = ({
                                     contribution,
                                     datasetId,
                                 }: { datasetId: string, contribution: ExpandedPullRequest }) => {
    return (
        <div className={ classes.itemContainer }>
            ...
            <div className={ classes.headerWrapper }>
                <div className={ classes.titleWrapper }>
                    { /* title */ }
                    <Link href={ `/marketplace/${datasetId}/contributions/${contribution._id}` }><p
                        className={ classes.title }>{ contribution.name }</p></Link>
                    { /* tags */ }
                    <div>
            ...
            <div className={ classes.footerWrapper }>
                <div className={ classes.itemContent }>
                    opened { dayjs(contribution.createdAt).fromNow() } by @{ contribution.user.username.slice(0, 16) }
                </div>
