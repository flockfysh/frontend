...
const ModalScreen = ({
  ...
  const [activeResource, setActiveResource] = useState<string>('All Notifications');

  const getNotifications = useCallback((key: string) => {
    ...
    notifications.forEach((notification) => {
      ...
      if (
        !notificationResourceList.find(
          (notificationResource) => notificationResource.name === resource
        )
      ) {
          ...
          <div className={ classes.content }>
            <div className={ classes.filters }>
              ...
              <div className={ classes.filtersList }>
                { getNotificationsList.map((notificationResource) => {
                  return (
                    <div
                      key={ notificationResource.name }
                      className={ classes.notificationListItem }
                    data-active={ activeResource === notificationResource.name }
                    onClick={ () => {
                      setActiveResource(notificationResource.name);
                    } }
                    >
            ...
            <div className={ classes.list }>
              { getNotifications(activeResource).map((notification, idx) => (
                <Notification
                setOpen={ setOpen }
