...
export default function ContributionDetails(props: {
  ...
  async function submitMessage(elem: HTMLFormElement) {
    ...
    if (fd.status) {
      await api.patch('/api/pullRequests/' + curContribution!._id + '/status', {
        status: fd.status,
      });
    }

    await api.post('/api/pullRequests/' + curContribution!._id + '/messages', {
      message: fd.comment,
    });
      ...
      { !showList && (
        <div className={ classes.assetViewContainer }>
          <div className={ classes.assetViewContainerHeader }>
            <button className={ classes.backButton } onClick={ toggleViewToList }>
              <BsArrowLeftCircle /> Back
            </button>
            <h3>{ curContribution?.name }</h3>
