...
const MultiFileInput = forwardRef<HTMLInputElement, FileInputProps>(function FileInput(props, ref) {
    let { classNames, children, maxFileCount, buttonLabel, ...inputProps } = props;

    const [fileDropOverlayVisible, setFileDropOverlayVisible] = useState(false);
    const [internalState, setInternalState] = useState<FileInputInternalState>(() => {
        return {
            files: new Map(),
        };
    });
    ...
    const dragOverTimeoutRef = useRef<NodeJS.Timeout | number | null>(null);
                ...
                <div className={ classes.inputRow }>
...

                    {
                        _maxFileCount !== Infinity ? (
                            ...
                            >
                                { internalState.files.size }/{ maxFileCount } files uploaded
                            </span>
                        )
