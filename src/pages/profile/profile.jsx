...
export default function Profile() {
   ...
   const [accounting, updateAccounts] = useState(null);
      ...
      <div className={ classes.containerDiv }>
         <div className={ classes.imageSection }>
            ...
            <div className={ classes.nameDiv }>
               <img src={ user.profileImage } alt="user profile" className={ classes.image } />
         ...
         <div className={ classes.cardSection }>
            <div className={ classes.card }>
               <h5 className={ classes.cardHeading }>Total Monthly Cost: ${ accounting.monthlyCosts.total }</h5>

               {
                  accounting.monthlyCosts.costs.map(
                     (cost, index) => (
                        <p className={ classes.cardText } key={ index }>
                           { cost.name } <span className={ classes.price }>${ cost.cost }</span>
                        </p>
                     )
                  )
               }
            ...
            <div className={ classes.card }>
...

               {
                  accounting.payments.map(
                     (payment, index) => (
                        <p className={ classes.cardText } key={ index }>
                           { payment.name } <span className={ classes.price }>${ payment.amount }</span>
                        </p>
                     )
                  )
               }
